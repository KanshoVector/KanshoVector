...
**データサイエンス学科（学部3年）｜数理モデリング × 本番デプロイ × AIネイティブ開発**
...
単なるスコア追求ではなく、複雑な時系列データや評価指標の制約を構造として読み解き、ロジックをシステムへ還元するプロセスを重視しています。

- **Hull Tactical (Featured/時系列予測):**
  - **Challenge:** 市場特有のノイズとボラティリティに対する頑健な予測。
  - **Approach:** トレンド分類器（方向性の特定）と投資配分回帰器（確信度の重み付け）を機能分離したアンサンブルモデルを設計、Kelly基準でAIの判定にブレーキ。
  - **Engineering:** データリークを排除した厳密な時系列クロスバリデーションパイプラインを構築し、汎化性能の確保に注力。
- **Road Accident Risk (予測):**
  - **Approach:** 地理的・環境的制約を考慮した特徴量エンジニアリングを優先。予測根拠の透明性を意識したモデル設計。

### 学習中
LLMオーケストレーション・システムアーキテクチャ
